...
const boxStyles: Record<InfoBoxType, { icon: typeof Info; colors: string; iconColor: string }> = {
  info: {
    icon: Info,
    colors: "bg-neon-blue/10 border-neon-blue/30",
    iconColor: "text-neon-blue",
  },
  hint: {
    icon: Lightbulb,
    colors: "bg-neon-cyan/10 border-neon-cyan/30",
    iconColor: "text-neon-cyan",
  },
  warning: {
    icon: AlertTriangle,
    colors: "bg-yellow-500/10 border-yellow-500/30",
  ...
  success: {
    icon: CheckCircle,
    colors: "bg-green-500/10 border-green-500/30",
    ...
    <div className={`my-6 rounded-xl border p-4 w-full ${style.colors}`}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-sm text-foreground/90 prose-p:my-2 prose-p:first:mt-0 prose-p:last:mb-0">
            {children}
          </div>
        </div>
      </div>
